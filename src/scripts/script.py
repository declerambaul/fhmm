...
for i in arange(0.5,1.4,0.1):
    stl = time()
    fastHMM = FastHMM(motif,index,cutoffWords=i,cutoffExtension=i)
    fastHMM.run()    
    tm.append(time()-stl)
...
xlabel("Cutoff threshold for both")
ylabel("Running Time (s)")
title("Time per cutoff threshold")

#show()
savefig("graphs/TimeCutoffBoth")

clf()
plot(arange(0.5,1.4,0.1),h)
xlabel("Cutoff threshold for both")
ylabel("Number of matches")
title("Number of matches per cutoff threshold")

#show()
savefig("graphs/NoMachtesCutoffBoth")
